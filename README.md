...
1. **Scraper** (`src/scraper.py`) відкриває сторінку кожного ASIN у headless Chromium, встановлює US zip-код через AJAX або UI-fallback для отримання правильної ціни buy-box, і парсить: назву, ціну, два BSR-рейтинги (основний + підкатегорія), рейтинг, кількість відгуків, купон, акцію, наявність.
...
4. **Slack** (`src/slack.py`) будує Block Kit повідомлення і надсилає в канал. При кількох групах між ними ставиться роздільник.

Пайплайн **stateless**: між запусками не зберігається жодних файлів — вся история в Google Sheets.
...
```yaml
marketplace: US
language: en_US                   # мова сторінок Amazon (впливає на URL запиту)
...
Щоб додати другу групу товарів — просто додай ще один блок `- name:`. Кожна група отримає окреме повідомлення в Slack, між ними буде розділювач.
...
git add .
git commit -m "init"
git remote add origin https://github.com/YOUR_USER/amazon-monitor.git
...
on:
  schedule:
    - cron: "35 8 * * *"   # 08:35 UTC = 11:35 за Молдовою/Києвом (EEST, UTC+3 влітку)
  workflow_dispatch: {}     # дозволяє запускати вручну з вкладки Actions
```

**Таблиця переведення часу:**

| Місцевий час (EEST, UTC+3) | UTC (для cron) |
|---|---|
| 09:00 | `0 6 * * *` |
| 10:00 | `0 7 * * *` |
| 11:35 | `35 8 * * *` |
...
> Workflow має вбудований таймаут 30 хвилин і захист від паралельних запусків (`concurrency`). Якщо попередній запуск ще не завершився, новий просто зачекає.
...
3. **URL:**
   ```
   https://api.github.com/repos/YOUR_USER/amazon-monitor/actions/workflows/monitor.yml/dispatches
   ```
...
### Зберегти дебаг-артефакти для будь-якого ASIN

Встанови змінну `DEBUG_SAVE=1` — скрейпер збереже скріншот і HTML для кожного ASIN незалежно від того, чи була CAPTCHA.
...
Скрейпер автоматично визначає CAPTCHA і робить retry з backoff — але не може вирішити CAPTCHA.  
